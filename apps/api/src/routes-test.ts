  ...
  router.post("/simulate-bank-flow", authenticate, async (req: Request, res: Response) => {
    ...
    if (!userId) {
      return res.status(404).json({
        message:
          "Usuario no encontrado en el sistema. Inicia sesión de nuevo o regístrate para usar la simulación SFA.",
      });
    }
    ...
    try {
      ...
      if (!webhookResult.success || !webhookResult.grant) {
        return res
          .status(500)
          .json({ message: "Webhook simulation failed", detail: webhookResult.error });
      }
