...
export function registerInstitutionRoutes(app: Express) {
  ...
  app.post(
    "/api/institutions/leads/:id/status",
    apiLimiter,
    authenticateInstitution,
    async (req: Request, res: Response) => {
      const institution = (req as AuthenticatedInstitutionRequest).institution!;
      ...
      if (!status || !VALID_RESPONSES.includes(status)) {
        return res
          .status(400)
          .json({ error: `status debe ser uno de: ${VALID_RESPONSES.join(", ")}` });
      }
      if (
        status === "originated" &&
        ...
        return res
          .status(400)
          .json({ error: "originated requiere originatedAmount > 0 (monto desembolsado)" });
      }
      try {
        const ok = await respondToLead(
        ...
        if (!ok) {
          return res
            .status(404)
            .json({ error: "Lead no encontrado o no pertenece a tu institución" });
        }
