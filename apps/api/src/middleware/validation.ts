...
export const updateExpenseSchema = z
  .object({
    amount: z.union([z.string().regex(/^\d+(\.\d{1,2})?$/), z.number().positive()]).optional(),
    name: z.string().max(200).optional().or(z.literal("")),
...
export const createBillSplitSchema = z
  .object({
...
export const updateBillSplitSchema = z
  .object({
    name: z.string().min(1).max(200).optional(),
    totalAmount: z.union([z.string().regex(/^\d+(\.\d{1,2})?$/), z.number().positive()]).optional(),
    description: z.string().max(1000).optional().or(z.literal("")),
    date: dateSchema.optional(),
    category: z.string().max(100).optional().or(z.literal("")),
