...
import { logAuthSecurityEvent, redactEmail } from "./authSecurityLog.js";
...
export function verifyPassword(password: string, storedHash: string): boolean {
  ...
  try {
    ...
    logger.warn(
      { err },
      "verifyPassword: error comparando hash (formato inválido o datos corruptos)",
    );
    return false;
  }
}
...
export async function storeOTP(email: string, code: string): Promise<void> {
  if (redis) {
    await redis.set(otpKey(email), JSON.stringify({ code, attempts: 0 }), "EX", OTP_TTL_SECONDS);
    return;
  }
...
export function invalidateToken(token: string, userId?: string): void {
  ...
  if (userId) {
    db.update(users)
      ...
      .catch((err: unknown) =>
        logger.error({ err, userId }, "Failed to persist token invalidation to DB"),
      );
  }
}
...
export async function authenticate(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Internal Server Error", message: "Authentication check failed" });
  }
}
...
function getAuthenticatedToken(req: AuthenticatedRequest): string | undefined {
  const authHeader = req.headers.authorization;
  return authHeader?.startsWith("Bearer ") ? authHeader.substring(7) : getTokenFromCookie(req);
}
 ...
 */
function registerClientMeta(req: Request): {
...
export async function handleRegister(req: Request, res: Response) {
  ...
  try {
    ...
    const purposeVersions: {
      purpose: (typeof REGISTRATION_REQUIRED_PURPOSES)[number];
...
export async function getOrCreateDemoUser(
  email: string,
): Promise<{ id: string; email: string; displayName?: string; username: string }> {
...
export async function handleRecoverMigrationPassword(req: Request, res: Response) {
  ...
  logger.debug(
    { emailProvided: !!body.email, secretConfigured: !!(secret && secret.length >= 16) },
    "[recovery] attempt",
  );
...
export async function handleLoginWithDB(req: Request, res: Response) {
  ...
  if (
    (!isProduction || demoModeEnabled) &&
    password === demoPassword &&
    isDemoAllowedEmail(email)
  ) {
      ...
      if (isTwoFactorEnabledFlag(user.twoFactorEnabled)) {
        ...
        if (!hasEmailConfig) {
          ...
          if (process.env.NODE_ENV === "production") {
            logger.error(
              { email: redactEmail(email) },
              "2FA enabled but no email service configured — failing closed. Set GMAIL_USER + GMAIL_APP_PASSWORD.",
            );
            return res.status(503).json({
              error: "Service Unavailable",
              code: "twofa_unavailable",
              message:
                "La verificación en dos pasos no está disponible temporalmente. Intenta más tarde o contacta a soporte.",
            });
          }
          logger.warn(
            { email: redactEmail(email) },
            "[DEV] 2FA skipped: no email service configured.",
          );
        } else {
          ...
          if (!sent) {
            ...
            logger.error(
              { email: redactEmail(email) },
              "2FA email failed/timed out — denying login (fail-closed)",
            );
            return res.status(503).json({
              error: "Service Unavailable",
              code: "twofa_email_failed",
              message:
                "No pudimos enviar tu código de verificación. Intenta nuevamente en unos minutos.",
            });
          } else {
            return res.json({
              success: true,
