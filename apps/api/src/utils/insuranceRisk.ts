...
const RISK_FACTORS = {
  ...
  ACCOUNT_BALANCE: 0.2, // Higher balances indicate lower risk
 ...
 */
export function computeInsuranceRiskFromFeatures(
  ...
  const healthScore =
    ...
    Math.max(
      ...
      Math.min(
        1,
        0.7 * features.incomeRegularity +
          0.3 * (features.activeDays / Math.max(1, features.windowDays)),
      ),
    );
  const healthRisk =
    healthScore < RISK_LEVELS.LOW ? "Low" : healthScore < RISK_LEVELS.MEDIUM ? "Medium" : "High";
  ...
  const propertyRisk =
    propertyScore < RISK_LEVELS.LOW
      ? "Low"
      : propertyScore < RISK_LEVELS.MEDIUM
        ? "Medium"
        : "High";
  ...
  const autoRisk =
    autoScore < RISK_LEVELS.LOW ? "Low" : autoScore < RISK_LEVELS.MEDIUM ? "Medium" : "High";
