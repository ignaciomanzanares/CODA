...
async function main() {
  ...
  for (const r of rows) {
    ...
    try {
      const raw =
        r.featuresSnapshot && looksEncrypted(r.featuresSnapshot)
          ? decryptField(r.featuresSnapshot)
          : r.featuresSnapshot;
  ...
  console.log(
    `Exportadas ${labeled} decisiones etiquetadas (de ${rows.length} snapshots) → ${outPath}`,
  );
  if (labeled === 0) {
    console.log(
      "Aún no hay outcomes etiquetados: se acumulan cuando las instituciones responden leads (reportLeadResponse).",
    );
  }
}
