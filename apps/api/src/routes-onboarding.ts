...
import { db, users, privacyConsentEvents } from './db/index.js';
...
async function hasDataProcessingConsent(uid: string): Promise<boolean> {
  ...
  const [row] = await db
    ...
    .where(and(eq(privacyConsentEvents.userId, uid), eq(privacyConsentEvents.purpose, 'data_processing')))
...
export function registerOnboardingRoutes(app: Express): void {
  ...
  app.get('/api/onboarding/status', authenticate, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const u = user as { kycStatus?: string | null; onboardingCompleted?: number; twoFactorEnabled?: number };
      return res.json({
    ...
    try {
      ...
      if (!consentGiven || !kycOk) {
        return res.status(400).json({
          message: 'Faltan pasos: registra el consentimiento y completa la verificación de identidad.',
          consentGiven,
 ...
 */
export async function requireTwoFactorForBankLink(uid: string): Promise<{ ok: true } | { ok: false; message: string }> {
  if (process.env.ENABLE_ONBOARDING !== 'true') return { ok: true };
  const user = await storage.getUser(uid);
  const enabled = Number((user as { twoFactorEnabled?: number } | undefined)?.twoFactorEnabled ?? 0) === 1;
