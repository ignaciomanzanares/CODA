  ...
  async listAccounts(_userId: string): Promise<OBAccount[]> {
    return [
      ...
      },
      {
        ...
        currency: "USD",
        mask: "5678",
        openedAt: daysAgo(720),
      },
      {
        providerAccountId: "mock-cc-001",
        name: "Credit Card",
        type: "credit_card",
        currency: "USD",
        mask: "4321",
        openedAt: daysAgo(200),
      },
    ];
  }
  ...
  async listTransactions(
    ...
    for (let i = 0; i < days; i += Math.ceil(Math.random() * 7)) {
      ...
      if (providerAccountId.includes("checking")) {
        arr.push({
          externalId: `${providerAccountId}-${i}`,
          postedAt: d,
          description: "Grocery Store",
          merchantName: "Whole Foods",
          amount: -45 - Math.random() * 60,
          currency: "USD",
          category: "Groceries",
        });
        arr.push({
          externalId: `${providerAccountId}-pay-${i}`,
          postedAt: d,
          description: "Salary",
        ...
        arr.push({
          externalId: `${providerAccountId}-int-${i}`,
          postedAt: d,
        ...
        arr.push({
          externalId: `${providerAccountId}-cc-${i}`,
          postedAt: d,
