...
describe("sfaMapper", () => {
  it("interno → SFA: salida = Débito, monto positivo, CLP entero, fecha ISO 8601 UTC", () => {
    const tx: InternalTx = {
      externalId: "TXN987654",
      postedAt: "2025-02-10T10:00:00Z",
      amount: -250000,
      currency: "CLP",
  ...
  it("interno → SFA: entrada = Crédito", () => {
    const r = toSfaTransaction({
      postedAt: "2025-02-12T14:25:00Z",
      amount: 300000,
      currency: "CLP",
      description: "Abono sueldo febrero",
      externalId: "TXN987655",
    });
  ...
  it("fecha-only se normaliza a medianoche UTC", () => {
    expect(
      toSfaTransaction({ postedAt: "2025-02-10", amount: 1, currency: "CLP", description: "x" })
        .bookingDateTime,
  ...
  it("round-trip SFA → interno → SFA preserva el ejemplo oficial", () => {
    ...
    const back = toSfaTransaction({
      externalId: ob.externalId,
      postedAt: ob.postedAt,
      amount: ob.amount,
  ...
  it("envelope: data.transactions + links(self/next) + meta(totalRecords/totalPages)", () => {
    const txs: InternalTx[] = Array.from({ length: 42 }, (_, i) => ({
      externalId: `T${i}`,
      postedAt: `2025-02-${String((i % 28) + 1).padStart(2, "0")}T00:00:00Z`,
      amount: i % 2 === 0 ? -1000 : 2000,
      currency: "CLP",
      description: `m${i}`,
    }));
...
describe("sfaMapper — operaciones de crédito", () => {
  ...
  const balance: SfaLoanBalance = {
    ...
    accruedInterest: 24000,
    accruedLateInterest: 0,
    nextInstallmentAmount: 131200,
    nextInstallmentDueDate: "2025-03-15",
    currency: "CLP",
    lastPaymentAmount: 131200,
  ...
  it("agrega señales de deuda para el evaluador de riesgo", () => {
    const ops = [
      ...
      fromSfaLoan(
        { ...loan, loanID: "LN-2", loanType: "HIPOTECARIO" },
        { ...balance, outstandingPrincipal: 40000000, accruedLateInterest: 1000 },
      ),
    ];
...
import {
  parseAmount,
  fromSfaAccountBalance,
  fromSfaCreditCard,
  fromSfaInvestment,
} from "../sfaMapper.js";
import type {
  SfaAccountBalance,
  SfaCreditCardBalance,
  SfaCreditCardLimit,
  SfaInvestment,
  SfaInvestmentBalance,
} from "../sfaTypes.js";
...
describe("sfaMapper — saldos cuenta/tarjeta/inversión", () => {
  ...
  it("saldo de cuenta (amount STRING) → saldo normalizado", () => {
    const b: SfaAccountBalance = {
      bookingDate: "2025-02-10T00:00:00Z",
      amount: "120000",
      currency: "CLP",
      available: "115000",
      lockedAmount: "5000",
  ...
  it("tarjeta de crédito: deuda facturada + utilización del cupo", () => {
    const balance: SfaCreditCardBalance = {
      closingDate: "2025-02-25",
      currency: "CLP",
      endOfMonthBalance: 1750000,
      minimumPaymentDue: 52000,
      dueDate: "2025-03-08",
      lastPaymentAmount: 200000,
      lastPaymentDate: "2025-01-10",
      debtType: "REVOLVING",
    };
    const limit: SfaCreditCardLimit = {
      totalApproved: 2500000,
      unusedLine: 750000,
      currency: "CLP",
      interestRate: 2.12,
      limitStartDate: "2023-05-01",
      limitEndDate: "2026-05-01",
      cashAdvancePercentage: 30,
  ...
  it("inversión → activo del usuario (usa valor actual del balance)", () => {
    const inv: SfaInvestment = {
      investmentType: "FONDOS_MUTUOS",
      rutClient: "12.345.678-9",
      productId: "INV-12345",
      productOwner: "Banco Ejemplo",
      financialProductType: "FONDOS_MUTUOS",
      commercialCategory: "Balanceado",
      coverageStartDate: "2023-02-01",
      coverageEndDate: null,
      insuredAmount: 0,
      insuredCurrency: "CLP",
      instrumentData: { fundSeries: "Clase A", mnemonic: "BAL-A", stockInvestment: 2500000 },
    };
    const bal: SfaInvestmentBalance = {
      amount: 2650000,
      currency: "CLP",
      updatedDateTime: "2025-02-28T23:59:00Z",
      holdAmount: 0,
      arrearsBalance: 0,
    };
  ...
  it("inversión sin balance usa stockInvestment del instrumentData", () => {
    const inv: SfaInvestment = {
      ...
      insuredAmount: 0,
      insuredCurrency: "CLP",
      instrumentData: { stockInvestment: 2500000 },
    };
