import type { DebtRule, DebtRuleContext } from './types.js';
...
const creditosGrandes = (cmf: CMFParseResult) =>
  cmf.deuda_directa.filter(
    (d) => (d.tipo_credito === 'vivienda' && d.vigente > 5_000_000) || (d.tipo_credito === 'consumo' && d.vigente > 1_000_000),
  );
...
export const DEBT_RULE_CATALOG: DebtRule[] = [
  ...
  },
  {
    ...
    accion: 'Destina tu excedente mensual a abonar/prepagar el crédito con la tasa más alta (método avalancha).',
  ...
  },
  {
    ...
    accion: 'Solicita ofertas de portabilidad financiera y compara la tasa (CAE) con tu crédito actual.',
  ...
  },
  {
    ...
    accion: 'Evita usar la línea de crédito/sobregiro para gastos corrientes; paga el saldo utilizado cuanto antes.',
  ...
  },
  {
    ...
    accion: 'Prioriza reducir saldos antes de tomar deuda nueva, hasta bajar del 30% de tu ingreso.',
  ...
  },
  {
    ...
    prioridad: 60,
    matches: (c) => c.health.ratios.ahorroIngreso < 0.1 && !c.health.ratios.moraActiva && c.health.nivel >= 0,
    describe: (c) =>
  ...
  },
  {
    ...
    accion: 'Si lo necesitas y puedes pagarlo sin esfuerzo, un producto de otro tipo bien manejado diversifica tu historial.',
  ...
  // ── Familia C — Ahorro y capacidad de pago ──
  {
    ...
    accion: 'Separa automáticamente un monto fijo cada mes hasta juntar 3 meses de gastos esenciales.',
  ...
  },
  {
    ...
    accion: 'Identifica tus 2–3 categorías de gasto no esencial más altas y reasígnalas a deuda o ahorro.',
  ...
  },
  {
    ...
    prioridad: 45,
    matches: (c) => c.health.ratios.ahorroIngreso >= 0.05 && c.health.ratios.ahorroIngreso < 0.2 && c.health.nivel >= 1,
    describe: (c) =>
    ...
    accion: 'Automatiza el ahorro apenas recibes tu ingreso (págate a ti primero), apuntando al 20%.',
