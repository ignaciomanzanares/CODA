...
function detectCardType(text: string): string {
  ...
  if (lower.includes('credito') || lower.includes('crédito') || lower.includes('tc ') || lower.includes('t.c.')) return 'credito';
  if (lower.includes('debito') || lower.includes('débito') || lower.includes('td ') || lower.includes('t.d.')) return 'debito';
  if (lower.includes('cuenta corriente') || lower.includes('cta cte') || lower.includes('cc ')) return 'cuenta_corriente';
  if (lower.includes('cuenta vista') || lower.includes('cta vista') || lower.includes('cv ')) return 'cuenta_vista';
...
const PATTERNS: ParserPattern[] = [
  ...
  {
    bank: 'Santander',
    pattern: /(?:compra|cargo|pago)(?:\s+por)?\s+\$?\s*([\d.,]+)\s+(?:en\s+)?(.+?)(?:\s+con\s+(.+?))?(?:\s+el\s+(\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?))?$/i,
    extract: (match, raw) => ({
  ...
  {
    bank: 'Banco de Chile',
    pattern: /(?:has\s+)?(?:realizado|hecho)\s+(?:una?\s+)?(?:compra|cargo|pago)\s+(?:por\s+)?\$?\s*([\d.,]+)\s+(?:en\s+)?(.+?)(?:\s+con\s+tu\s+tarjeta)?/i,
    extract: (match, raw) => ({
      amount: parseAmount(match[1]),
      merchant: match[2]?.trim().replace(/\s+con.*$/i, '').replace(/\s+/g, ' ') || 'Desconocido',
      cardLastDigits: extractLastDigits(raw),
      operationType: 'cargo',
    }),
  },
  ...
  {
    bank: 'BCI',
    pattern: /(?:bci[:\s]+)?(?:compra\s+aprobada|cargo|pago)\s+\$?\s*([\d.,]+)\s+(.+?)(?:\s+(?:tc|td|débito|debito|credito|crédito)\s*)?(?:\*+\d{4})?(?:\s+(\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?))?$/i,
    extract: (match, raw) => ({
      amount: parseAmount(match[1]),
      merchant: match[2]?.trim().replace(/\s+(?:tc|td|débito|debito|credito|crédito).*$/i, '').replace(/\s+/g, ' ') || 'Desconocido',
  ...
  {
    bank: 'BancoEstado',
    pattern: /(?:bancoestado[:\s]+)?(?:compra|cargo|pago)\s+\$?\s*([\d.,]+)\s+(?:en\s+)?(.+?)(?:\s+con\s+(.+))?$/i,
    extract: (match, raw) => ({
      amount: parseAmount(match[1]),
      merchant: match[2]?.trim().replace(/\s+con.*$/i, '').replace(/\s+/g, ' ') || 'Desconocido',
      cardLastDigits: extractLastDigits(raw),
  ...
  {
    bank: 'Scotiabank',
    pattern: /(?:scotia(?:bank)?[:\s]+)?(?:compra|cargo|pago)\s+(?:por\s+)?\$?\s*([\d.,]+)\s+(?:en\s+)?(.+?)(?:\s+(?:tc|td))?(?:\s*\*+\d{4})?$/i,
    extract: (match, raw) => ({
      amount: parseAmount(match[1]),
      merchant: match[2]?.trim().replace(/\s+(?:tc|td).*$/i, '').replace(/\s+/g, ' ') || 'Desconocido',
      cardLastDigits: extractLastDigits(raw),
      operationType: 'cargo',
    }),
  },
