...
import type { CreditScorePredictionLog } from "./algorithmicTraceability.js";
import { getActiveModelVersion, registerAlgorithmChange } from "./algorithmicTraceability.js";
 ...
 */
function computePSI(expectedDist: number[], actualDist: number[]): number {
...
export function monitorModelDrift(
  ...
  const alertTriggered =
    overallDriftScore > 0.3 || featureDrift.some((f) => f.alert) || stdScore > 100;
  ...
  const alertReason = alertTriggered
    ? [
        ...
        featureDrift.some((f) => f.alert) &&
          `Feature drift detected: ${featureDrift
            .filter((f) => f.alert)
            .map((f) => f.featureName)
            .join(", ")}`,
...
export async function runDriftMonitoringJob(opts?: {
  ...
  logger.info(
    { kind, psi, alert, recentN: recent.length, referenceN: reference.length },
    "[driftMonitor] PSI calculado",
  );

  if (alert) {
    await notifyOps(
      `Drift de PD detectado en '${kind}' (PSI=${psi.toFixed(3)} > ${PSI_DRIFT_THRESHOLD})`,
      {
        kind,
        psi,
        recentN: recent.length,
    ...
    if (psi > PSI_SEVERE_THRESHOLD) {
      ...
      logger.warn(
        { kind, psi },
        "[driftMonitor] drift severo — AlgorithmChange pendiente registrado",
      );
    }
  }
...
export function exampleDriftCheck() {
  ...
  console.log(
    `  Period: ${driftResult.periodStart.toISOString()} - ${driftResult.periodEnd.toISOString()}`,
  );
