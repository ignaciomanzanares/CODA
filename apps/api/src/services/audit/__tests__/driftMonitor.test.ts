...
describe('runDriftMonitoringJob (DB-backed, #24)', () => {
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < N; i++) {
      await insertLog(0.04 + (i % 3) * 0.01, `2026-01-01T00:00:${String(i % 60).padStart(2, '0')}.000Z`, i);
    }
    // Lote RECIENTE (createdAt más nuevo): PD desplazado a ~0.6 (drift fuerte).
    for (let i = 0; i < N; i++) {
      await insertLog(0.58 + (i % 3) * 0.01, `2026-02-01T00:00:${String(i % 60).padStart(2, '0')}.000Z`, N + i);
    }
  });
