...
import { detectGovSource, parseGovDocument, parseAfp, parseSii, parseTgr, extractMontosClp } from '../govParsers';
...
describe('govParsers', () => {
  ...
  it('detectGovSource clasifica por puntaje de marcadores', () => {
    expect(detectGovSource('A.F.P. Modelo certifica CUENTA OBLIGATORIA — COTIZACION NORMAL')).toBe('afp');
    expect(detectGovSource('SERVICIO DE IMPUESTOS INTERNOS - CARPETA TRIBUTARIA - FORMULARIO 22')).toBe('sii');
    expect(detectGovSource('El Servicio de Tesorería certifica. Cuenta Única Tributaria. NO REGISTRA DEUDA')).toBe('tgr');
  ...
  it('detectGovSource: la carpeta SII gana aunque mencione «cotizaciones previsionales»', () => {
    ...
    const carpeta = 'SERVICIO DE IMPUESTOS INTERNOS. CARPETA TRIBUTARIA. FORMULARIO 22. ' +
  ...
  it('TGR: "no registra deuda" → fiscalDebtClp 0', () => {
    const r = parseTgr('El Servicio de Tesorería certifica que el RUT NO REGISTRA DEUDA fiscal vigente.');
