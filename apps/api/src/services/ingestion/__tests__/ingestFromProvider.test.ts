...
describe("ingestFromProvider (#18)", () => {
  ...
  it("CartolaUploadProvider mapea una cartola a la misma forma de datos", async () => {
    ...
    const summary = await ingestFromProvider(
      userId,
      new CartolaUploadProvider("Banco Estado", [cartola]),
    );
    ...
    const again = await ingestFromProvider(
      userId,
      new CartolaUploadProvider("Banco Estado", [cartola]),
    );
