...
function makeHealth(over: Partial<HealthEvaluationResult['ratios']> & { nivel?: number }): HealthEvaluationResult {
  return {
...
describe('habit effectiveness (#34)', () => {
  ...
  it('registra snapshot y mide mejora contra el ciclo siguiente', async () => {
    ...
    const cycle2 = makeHealth({ deudaFlujo: 0.40 });
    ...
    expect(deudaProgress!.after).toBeCloseTo(0.40, 5);
