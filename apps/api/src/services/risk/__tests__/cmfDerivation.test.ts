...
function cmf(overrides: Partial<CMFParseResult> = {}): CMFParseResult {
  return {
    titular: '', rut: '', fecha_emision: new Date(0), fecha_actualizacion: new Date(0),
    deuda_total: 0, deuda_directa: [], deuda_indirecta: [], lineas_credito: [],
    metricas: { tiene_deuda: false, porcentaje_al_dia: 100, porcentaje_en_atraso: 0, tiene_atraso_grave: false, score_cmf: 85, credito_disponible_total: 0 },
    ...overrides,
  } as CMFParseResult;
}

const row = (o: Partial<CMFParseResult['deuda_directa'][number]>) => ({
  institucion: 'X', tipo_credito: 'consumo' as const, total: 0, vigente: 0,
  atraso_30_59: 0, atraso_60_89: 0, atraso_90_mas: 0, ...o,
});
...
describe('cmfDerivation', () => {
  ...
  it('deriveCmfFeatures: utilización = deuda / (deuda + disponible)', () => {
    const c = cmf({ deuda_total: 800_000, metricas: { ...cmf().metricas, credito_disponible_total: 200_000 } });
  ...
  it('deriveCmfFeatures: cuenta tipos de crédito distintos', () => {
    const c = cmf({
      deuda_directa: [row({ tipo_credito: 'consumo' }), row({ tipo_credito: 'vivienda' }), row({ tipo_credito: 'consumo' })],
    });
