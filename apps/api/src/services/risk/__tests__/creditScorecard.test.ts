...
describe('creditScorecard (logístico GMSC)', () => {
  it('paridad con el modelo Python entrenado (ancla los coeficientes)', () => {
    ...
    expect(evaluateCreditScorecard({ mora30: 0, mora60: 0, mora90: 0, dti: 0.2, util: 0.1 }).pd * 100).toBeCloseTo(2.31, 1);
    expect(evaluateCreditScorecard({ mora30: 1, mora60: 0, mora90: 0, dti: 0.5, util: 0.5 }).pd * 100).toBeCloseTo(7.47, 1);
    expect(evaluateCreditScorecard({ mora30: 2, mora60: 1, mora90: 1, dti: 1.5, util: 0.9 }).pd * 100).toBeCloseTo(35.33, 1);
  });
  ...
  it('score se mantiene en [0, 850]', () => {
    ...
    const worst = evaluateCreditScorecard({ mora30: 10, mora60: 10, mora90: 10, dti: 2, util: 2 }).score;
