...
describe("anonymizeUser", () => {
  ...
  it("deletes user assets, inscripcion jobs, assistant memory/feedback, habit feedback, parse diagnostics and conversion events", async () => {
    expect(await db.select().from(userAssets).where(eq(userAssets.userId, USER_ID))).toHaveLength(
      0,
    );
    expect(
      await db.select().from(inscripcionJobs).where(eq(inscripcionJobs.userId, USER_ID)),
    ).toHaveLength(0);
    expect(
      await db.select().from(assistantSummaries).where(eq(assistantSummaries.userId, USER_ID)),
    ).toHaveLength(0);
    expect(
      await db.select().from(assistantFeedback).where(eq(assistantFeedback.userId, USER_ID)),
    ).toHaveLength(0);
    expect(
      await db.select().from(habitFeedback).where(eq(habitFeedback.userId, USER_ID)),
    ).toHaveLength(0);
    expect(
      await db
        .select()
        .from(documentParseOutcomes)
        .where(eq(documentParseOutcomes.userId, USER_ID)),
    ).toHaveLength(0);
    expect(
      await db.select().from(parserDiagnostics).where(eq(parserDiagnostics.userId, USER_ID)),
    ).toHaveLength(0);
    expect(
      await db
        .select()
        .from(productConversionEvents)
        .where(eq(productConversionEvents.userId, USER_ID)),
    ).toHaveLength(0);
  });
  ...
  it("de-links audit_logs from the user instead of deleting them", async () => {
    const [log] = await db
      .select()
      .from(auditLogs)
      .where(eq(auditLogs.action, `login-anon-test-${USER_ID}`));
  ...
  it("keeps the algorithm_prediction_logs row but de-links userId and strips input PII", async () => {
    const logs = await db
      ...
      .where(eq(algorithmPredictionLogs.requestId, REQUEST_ID));
  ...
  it("keeps the consent grant/event rows but anonymizes IP/user-agent", async () => {
    ...
    const [event] = await db
      ...
      .where(eq(privacyConsentEvents.userId, USER_ID));
