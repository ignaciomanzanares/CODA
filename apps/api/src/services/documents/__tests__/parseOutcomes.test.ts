...
describe('parseOutcomes (#32)', () => {
  ...
  it('registra un outcome de éxito con metadata', async () => {
    ...
    const rows = await db.select().from(documentParseOutcomes).where(sql`user_id = ${userId}`);
  ...
  it('registra un fallo y produce un mensaje accionable para el usuario', async () => {
    await recordParseOutcome(userId, { status: 'failed', documentType: 'cartola', errorMessage: 'no se detectó formato' });
    const rows = await db
      .select()
