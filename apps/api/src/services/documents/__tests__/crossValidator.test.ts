...
function cmf(partial: Partial<CMFParseResult>): CMFParseResult {
  return {
    titular: 'x', rut: '1-9', fecha_emision: new Date(), fecha_actualizacion: new Date(),
    deuda_total: 0, deuda_directa: [], deuda_indirecta: [], lineas_credito: [],
    metricas: { tiene_deuda: false, porcentaje_al_dia: 100, porcentaje_en_atraso: 0,
      tiene_atraso_grave: false, score_cmf: 85, credito_disponible_total: 0 },
    ...partial,
  };
}
...
describe('crossValidateCmfVsCartolas', () => {
  ...
  it('marca CMF sin deuda contradicho por servicio de deuda en cartola', async () => {
    listMock.mockResolvedValue([{ parsedData: { banco: 'BancoEstado', transacciones: [
    ...
    const w = await crossValidateCmfVsCartolas('u1', cmf({ deuda_total: 0, metricas: { ...cmf({}).metricas, tiene_deuda: false } }));
  ...
  it('marca cobertura: deuda CMF en institución sin cartola', async () => {
    ...
    const w = await crossValidateCmfVsCartolas('u1', cmf({
      ...
      deuda_directa: [{ institucion: 'Banco Santander', tipo_credito: 'consumo', total: 5_000_000, vigente: 5_000_000, atraso_30_59: 0, atraso_60_89: 0, atraso_90_mas: 0 }],
    }));
