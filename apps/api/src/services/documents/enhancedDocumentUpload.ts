...
import { validateDocument, validateDocumentWithContent, validateDocumentType } from './documentValidator.js';
...
export async function processDocumentRobust(
  ...
  try {
    ...
    if (file.mimetype === 'application/pdf') {
      ...
      if (doc) {
        ...
        if (textLength >= 100) {
          ...
          // Validate content
          const contentValidation = await validateDocumentWithContent(
            file,
            (doc as any).rawText
          );
          ...
          logger.warn(
            { textLength },
            '[EnhancedUpload] Insufficient text extracted, will try OCR'
          );
        }
      }
    }
    ...
    if (enableOcr) {
      ...
      try {
        const ocrResult = await smartOcr(
          file.buffer,
          extractedText,
          file.mimetype
        );
