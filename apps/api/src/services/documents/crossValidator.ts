...
const DEBT_SERVICE_RE = /inter[eé]s|intereses|cuota|dividendo|pr[eé]stamo|cr[eé]dito|l[ií]nea de cr[eé]dito|pago.*tarjeta|tarjeta de cr[eé]dito/i;
...
export async function crossValidateCmfVsCartolas(
  ...
  try {
    ...
    const parsed = cartolas
      .map((r) => r?.parsedData as { transacciones?: Array<Record<string, unknown>>; banco?: string } | null)
    ...
    if (cmf.deuda_total > 0 && cartolaBancos.size > 0) {
      ...
      const faltantes = [...cmfInstituciones].filter(
        (inst) => inst.length > 0 && ![...cartolaBancos].some((cb) => cb.includes(inst) || inst.includes(cb)),
      );
