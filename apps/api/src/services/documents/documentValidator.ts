 ...
 */
function validatePdfIntegrity(buffer: Buffer): { valid: boolean; error?: string; warning?: string } {
...
function detectIfScanned(text: string, pageCount: number): boolean {
  ...
  const hasOcrArtifacts = /[▪●◦■□▫▪]/g.test(text) || // Bullet points often fail OCR
...
export async function validateDocumentWithContent(
  ...
  if (extractedText) {
    ...
    const hasCmfIndicators = /CMF|Comisión.*Mercado.*Financiero|Informe.*Deudas/i.test(extractedText);
    const hasCartolaIndicators = /Cartola|Banco.*Santander|Banco.*Chile|Estado.*Cuenta/i.test(extractedText);
...
export function validateDocumentType(
  ...
  if (!extractedDoc) {
    return {
      valid: false,
      error: 'No se pudo extraer información del documento. Verifica que sea un PDF válido con texto.',
    };
  }
  ...
  if (extractedDoc.tipo === 'cmf_informe_deudas') {
    if (!extractedDoc.rutDocumento) {
      return {
        valid: false,
        error: 'No se encontró un RUT válido en el Informe CMF. Verifica que sea un documento oficial.',
      };
    }
    
    if (
      extractedDoc.deudaTotalVigente === undefined ||
      extractedDoc.deudaTotalVigente === null
    ) {
      return {
        valid: false,
  ...
  if (extractedDoc.tipo === 'cartola') {
    if (!extractedDoc.transacciones || extractedDoc.transacciones.length === 0) {
      return {
        valid: false,
        error: 'No se encontraron transacciones en la cartola. Verifica que el documento sea correcto.',
      };
    }
