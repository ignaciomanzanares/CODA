...
async function main() {
  ...
  console.log(
    `[promote-model] modelId=${modelId} auc=${auc.toFixed(4)} keyPrefix=${keyPrefix} lifecycle=${lifecycle} mode=${apply ? "APPLY" : "dry-run"}`,
  );

  if (auc < 0.6) {
    console.error(
      `[promote-model] AUC ${auc.toFixed(4)} < 0.60 — abortando (no se registra un modelo peor que el umbral).`,
    );
    process.exit(1);
  }
  ...
  if (!apply) {
    console.log(
      `[promote-model] dry-run: subiría los 3 artefactos al blob store y registraría esta versión como '${lifecycle}'. Re-ejecuta con --apply.`,
    );
    return;
  }
  ...
  for (const f of ARTIFACT_FILES) {
    ...
    const uri = await store.putObject(`${keyPrefix}/${f}`, bytes, {
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(algorithmModelVersions.modelType, MODEL_TYPE),
        ),
      );
  }
