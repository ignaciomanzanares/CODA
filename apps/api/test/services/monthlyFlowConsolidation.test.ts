...
const MONTH: StoredTx[] = [
  ...
  // ── plomería interna (debe netear a cero; ambas patas) ──
  {
    ...
    tipo: "cargo",
    monto: 478_746,
    descripcion: "Agustinas Traspaso Internet a T. Crédito",
  ...
  }, // CC → fondeo tarjeta
  {
    ...
    tipo: "cargo",
    monto: 390_139,
    descripcion: "Agustinas Egreso por Compra de Divisas",
    es_transferencia: true,
    category: "Inversión y divisas",
  }, // CC → divisas tarjeta USD
  {
    fecha: "2026-05-08",
    tipo: "abono",
    monto: 478_746,
  ...
  }, // pago tarjeta (otra pata)
  {
    ...
    es_transferencia: true,
    category: "Inversión y divisas",
  }, // divisas USD (otra pata)
  // ── consumo real de tarjeta (debe quedar, una vez cada uno) ──
  {
    fecha: "2026-05-10",
    tipo: "cargo",
    monto: 482_053,
    descripcion: "PAGO COOPEUCH",
    es_transferencia: false,
    category: "Vivienda",
  }, // dividendo hipotecario — REAL
  {
    fecha: "2026-05-11",
    tipo: "cargo",
    monto: 33_400,
    descripcion: "STA ISABEL PROV BILBAO",
    es_transferencia: false,
    category: "Supermercado y almacén",
  }, // compra tarjeta — REAL
  {
    ...
    tipo: "cargo",
    monto: 22_830,
    descripcion: "OPENAI *CHATGPT SUBSCR",
    es_transferencia: false,
...
describe("monthly-flow consolidado (UI)", () => {
  it("netea el fondeo/divisas y conserva el consumo real una sola vez", () => {
    ...
    expect(
      MONTH.filter((t) => /COOPEUCH/.test(t.descripcion) && !isInternalTransferTx(t)),
    ).toHaveLength(1);
  });
  ...
  it("NO netea transferencias a terceros aunque vengan con es_transferencia=true", () => {
    ...
    const terceroRecibida: StoredTx = {
      fecha: "2026-05-14",
      tipo: "abono",
      monto: 60_000,
      descripcion: "Transferencia de Maria Soto",
      es_transferencia: true,
      category: "Transferencias",
    };
  ...
  it("reconoce la plomería interna por glosa, categoría y flag", () => {
    ...
    expect(isInternalTransferTx({ descripcion: "pago", category: "Transferencia interna" })).toBe(
      true,
    );
