...
describe('unified flow consolidation', () => {
  it('classifies internal plumbing vs real consumption correctly', () => {
    ...
    for (const keep of ['PAGO COOPEUCH', 'STA ISABEL PROV BILBAO', 'OPENAI *CHATGPT SUBSCR', 'Pago Sueldo Empresa SpA', 'Jumbo Bilbao']) {
  ...
  it('nets the card-funding transfer and keeps card purchases once', () => {
    ...
    expect(MONTH.filter((t) => /COOPEUCH/.test(t.description) && !isInternalTransfer(t))).toHaveLength(1);
  });
});
