...
describe('merchantCategorizer — transferencias internas (máxima prioridad)', () => {
  ...
  it('una remuneración real sigue siendo Ingresos', () => {
    const r = categorize({ descripcion: 'PAGO SUELDO EMPRESA SPA', monto: 2_500_000, tipo: 'abono' });
...
describe('isInternalTransferTx — exclusión consolidada', () => {
  it('true cuando la categoría persistida es Transferencia interna', () => {
    expect(isInternalTransferTx({ categoria: 'Transferencia interna', descripcion: 'lo que sea' })).toBe(true);
  });
  ...
  it('true por flag es_transferencia del adaptador de tarjeta', () => {
    expect(isInternalTransferTx({ descripcion: 'MONTO CANCELADO', es_transferencia: true })).toBe(true);
  });

  it('false para transferencia a tercero (se cuenta), incluso con es_transferencia', () => {
    expect(isInternalTransferTx({ descripcion: 'Transf a Juan Perez', es_transferencia: true })).toBe(false);
  });
});
...
describe('aceptación — netea ingreso Y egreso, conserva terceros', () => {
  ...
  type Tx = { descripcion: string; tipo: 'abono' | 'cargo'; monto: number; categoria?: string; es_transferencia?: boolean };
  const set: Tx[] = [
    { descripcion: 'Pago Sueldo Empresa SpA', tipo: 'abono', monto: 2_500_000, categoria: 'ingreso_principal' },
    { descripcion: 'MONTO CANCELADO', tipo: 'abono', monto: 478_746, categoria: 'Transferencia interna', es_transferencia: true },
    { descripcion: 'ABONO DE DIVISAS', tipo: 'abono', monto: 390_139, categoria: 'Transferencia interna', es_transferencia: true },
    { descripcion: 'Traspaso Internet a T. Crédito', tipo: 'cargo', monto: 478_746, categoria: 'Transferencia interna', es_transferencia: true },
    { descripcion: 'Transf a Maria', tipo: 'cargo', monto: 60_000, categoria: 'transferencia_enviada', es_transferencia: true },
