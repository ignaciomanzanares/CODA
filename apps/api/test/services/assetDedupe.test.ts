...
import { assetSignature, normalizeAssetDescription, effectiveAssetValueClp, type UserAsset } from '../../src/services/assets/types.js';
...
describe('assetSignature (guard idempotente + dedup)', () => {
  ...
  it('trata garantía ausente y garantía 0 como equivalentes, pero distinta de una garantía real', () => {
    expect(assetSignature({ ...base, lienAmountClp: null })).toBe(assetSignature({ ...base, lienAmountClp: 0 }));
...
describe('Deuda/Activos tras quitar el activo duplicado (47% → 90%)', () => {
  const propiedad: UserAsset = {
    id: 'a1', userId: 'u1', type: 'property', name: 'Depto 306 Bilbao',
    acquisitionCostClp: 100_000_000, estimatedValueClp: 100_000_000,
    hasLien: true, lienAmountClp: 70_000_000, currency: 'CLP',
    documentId: null, notes: null, createdAt: '2025-01-01', updatedAt: '2025-01-01',
  };
  ...
  const cmf = { deuda_total: deudaTotalClp, deuda_directa: [], deuda_indirecta: [], lineas_credito: [], metricas: { porcentaje_al_dia: 100, score_cmf: 85, tiene_mora: false, credito_disponible_total: 0, utilizacion_promedio: 0 } } as any;

  function ratio(assets: UserAsset[]): number {
    return deriveHealthInput({
      ingresoMensualClp: 3_000_000, deudaMensualClp: deudaTotalClp / 36, deudaTotalClp,
      ahorroMensualClp: 500_000, cmf, sfaProductBalancesClp: liquidosClp, userAssets: assets,
    }).deudaActivos;
  }
  ...
  it('ANTES (con duplicado) ≈ 47%, DESPUÉS (deduplicado) ≈ 90%', () => {
    ...
    console.log(`Deuda/Activos  ANTES=${(before * 100).toFixed(1)}%  DESPUÉS=${(after * 100).toFixed(1)}%`);

    expect(before).toBeCloseTo(0.47, 2);
    expect(after).toBeCloseTo(0.90, 2);
