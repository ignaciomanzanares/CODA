...
describe('card statement → ParseResult adapter', () => {
  ...
  (has(OCT) ? it : it.skip)('CLP: purchases become cargos summing to $596.864; payments netted', async () => {
  ...
  (has(MAR) ? it : it.skip)('USD: purchases convert to CLP via injected getUsd; payments tagged internal', async () => {
    const rate = 950;
  ...
  (has(MAR) ? it : it.skip)('USD: fx unavailable → never blocks, flags FX pendiente, monto CLP 0', async () => {
