...
describe("CBR inscription extractor — depto E102 (Dominio only)", () => {
  ...
  (present ? it : it.skip)("extracts Dominio + compraventa and returns mortgage=null", () => {
    ...
    expect(r.warnings).not.toContain(
      "Documento de Hipoteca presente pero no se pudo leer el monto UF.",
    );
  ...
  (present ? it : it.skip)(
    "resolveInscripcionPrefill fetches UF at the escritura (Dominio) date",
    async () => {
      const r = extractInscripcion(readFileSync(fxFile, "utf8"));
      const calls: string[] = [];
      ...
      console.log(
        `[E102] resolved acquisitionCostClp=${prefill.acquisitionCostClp} (escrituraDate=${prefill.source.escrituraDate})`,
      );
...
describe("compraventa deletreada cuando el OCR mutila los dígitos", () => {
  it('rescata la compraventa desde la forma en palabras anclada a "precio"', () => {
    const snippet =
      "En el precio de DOS MIL QUINIENTAS CINCUENTA Y NUEVE Unidades de Fomento, pagado al contado.";
    const r = extractInscripcion(snippet);
    expect(r.compraventaUf).toBe(2559);
  });
});
...
describe("depto-306 — texto OCR real (estructura repartida; campos + prefill sin throw)", () => {
  ...
  (OCR_306 ? it : it.skip)(
    "extrae hipoteca/lender/registro/rol/dominio y arma el prefill (compraventa null tolerada)",
    () => {
  ...
  (OCR_306 ? it : it.skip)(
    "extractInscripcionFromBuffer devuelve un resultado parcial usable (no manualFallback)",
    async () => {
...
describe('buildAssetPrefill — null-safe, nunca lanza (fix "referencia")', () => {
  it("no lanza con un result hueco/parcial (dominio undefined)", () => {
    ...
    const p = buildAssetPrefill(
      { hipoteca: { acreedor: "COOPEUCH", montoUf: 100 } } as any,
      38_000,
    );
...
describe("extractInscripcionFromBuffer — OCR fallback (injected deps, no real OCR)", () => {
  ...
  (e102Text ? it : it.skip)(
    "falls back to OCR when the text layer is empty (scanned bundle)",
    async () => {
      const out = await extractInscripcionFromBuffer(buf, {
...
describe("extractInscripcionFromBuffer — skipOcr (fast-path vs job)", () => {
  ...
  (e102Text ? it : it.skip)(
    "capa de texto suficiente → ok, sin needsOcr, sin correr OCR",
    async () => {
      const out = await extractInscripcionFromBuffer(buf, {
        extractText: async () => e102Text,
        ocr: async () => {
          throw new Error("OCR no debe correr en el fast-path");
        },
        skipOcr: true,
      });
      expect(out.ok).toBe(true);
