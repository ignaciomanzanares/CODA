...
import {
  ...
  getEmpresasReconciliation,
  runEmpresasReconciliation,
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {(companies ?? []).map((c: { id: number; name: string }) => (
          <a
            key={c.id}
            href={`/empresas/reconciliation?company_id=${c.id}`}
          ...
          {isLoading ? (
            ...
            <Card>
              <CardContent className="pt-6">
                <p>
                  <strong>Emparejados:</strong> {summary.summary.matchedCount} de{" "}
                  {summary.summary.totalTransactions} transacciones.
                </p>
                <p className="text-muted-foreground">
                  Sin emparejar: {summary.summary.unmatchedTransactionCount}.
                </p>
              </CardContent>
            </Card>
          ) : (
            <Card>
              <CardContent className="py-8 text-center text-muted-foreground">
                Sin datos de reconciliación. Ejecuta la reconciliación arriba.
              </CardContent>
            </Card>
          )}
        </>
