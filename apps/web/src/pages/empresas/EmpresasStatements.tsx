...
export default function EmpresasStatements() {
  ...
  const [thresholdClp, setThresholdClp] = useState(() => (companyId ? getStoredThreshold(companyId) : DEFAULT_THRESHOLD_CLP));
  useEffect(() => {
  ...
  const cashAlerts = useMemo(() => {
    ...
    return (cashForecast.forecast as { date: string; projectedBalance: number }[]).filter((f) => f.projectedBalance < th);
      ...
      {cashAlerts.length > 0 && (
        <Alert variant="destructive" className="flex items-start gap-2">
          ...
          <div>
            ...
            <AlertDescription>
              La proyección de caja cae bajo tu umbral mínimo ({formatCurrency(thresholdClp, "CLP", { sourceCurrency: "CLP" })}) en {cashAlerts.length} día(s):{" "}
              {cashAlerts.slice(0, 5).map((a: { date: string; projectedBalance: number }) => `${a.date} (${formatCurrency(a.projectedBalance, currency, { sourceCurrency: "CLP" })})`).join(", ")}
          ...
          {cashForecast?.forecast?.length ? (
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardDescription>Asistente de flujo de caja — saldo proyectado según histórico. Configura un umbral mínimo para recibir alertas.</CardDescription>
              </CardHeader>
              <CardContent className="text-sm space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  <Label htmlFor="cash-threshold" className="text-muted-foreground">Umbral mínimo (CLP):</Label>
                  <Input
                    id="cash-threshold"
                    type="text"
                    inputMode="numeric"
