...
export default function EmpresasPurchaseOrders() {
  ...
  const linkMutation = useMutation({
    mutationFn: ({ poId, dteId }: { poId: number; dteId: number }) =>
      linkPurchaseOrderToDte(poId, dteId),
    onSuccess: () => {
            ...
            <TabsContent value="all" className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Vincule una OC a un DTE recibido para marcar como facturado.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {!orders?.length ? (
                    <p className="text-muted-foreground text-sm py-4">
                      No hay órdenes de compra. Sincronice desde Conectores → Órdenes de Compra.
                    </p>
                  ) : (
                        ...
                        <tbody>
                          {(orders as PurchaseOrder[]).map((po) => (
                            <tr key={po.id} className="border-b last:border-0">
                              ...
                              <td className="py-2 pr-2 text-right">
                                {formatCurrency(po.invoicedAmount ?? 0, currency, {
                                  sourceCurrency: "CLP",
                                })}
                              </td>
                        ...
                        <div key={v.vendorRut} className="border rounded-lg p-4">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm text-muted-foreground ml-auto">
                              Total:{" "}
                              {formatCurrency(v.totalAmount, currency, { sourceCurrency: "CLP" })}
                            </span>
                          </div>
          ...
          <Dialog open={linkPoId != null} onOpenChange={(open) => !open && setLinkPoId(null)}>
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Seleccione la factura recibida (DTE) que corresponde a esta orden de compra. El
                  estado se actualizará a facturado.
                </DialogDescription>
              </DialogHeader>
              <div className="py-2">
                {receivedDtes.length === 0 ? (
                  <p className="text-muted-foreground text-sm">
                    No hay documentos recibidos. Sincronice con SII o ingrese DTE recibidos.
                  </p>
                ) : (
              ...
              <DialogFooter>
                ...
                <Button
                  ...
                  onClick={() =>
                    linkPoId != null &&
                    linkDteId != null &&
                    linkMutation.mutate({ poId: linkPoId, dteId: linkDteId })
                  }
                >
