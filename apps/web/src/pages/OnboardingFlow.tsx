...
export default function OnboardingFlow() {
  ...
  const canAdvance =
    current === "account"
      ? name.trim().length > 1 && /\S+@\S+\.\S+/.test(email) && pwdValid && pwdMatch && consent
      : current === "terms"
        ? consent
        : current === "kyc"
          ? front && back && bio
          : current === "2fa"
            ? twoFA !== null && (twoFA !== "now" || totpDone)
            : false;
  ...
  const buttonLabel =
    current === "account" ? "Crear cuenta" : isLastStep ? "Finalizar" : "Continuar";
      ...
      <div className="flex-1 lg:w-1/2 flex flex-col justify-center bg-background px-6 py-12 sm:px-12 lg:px-16 overflow-y-auto">
        ...
        <div className="w-full max-w-md mx-auto">
          ...
          {error && (
            <div
              role="alert"
              className="mb-5 rounded-xl border border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-500/10 px-4 py-3 text-sm text-red-600 dark:text-red-400"
            >
              {error}
            </div>
          )}
          ...
          <div className="flex gap-3 mt-8">
            ...
            >
              {busy ? (
                ...
                <>
                  {buttonLabel} <ChevronRight size={17} />
                </>
              )}
            </button>
          </div>
...
function ConsentCheckbox({
  ...
  return (
    <label
      className={cn(
        "flex gap-3 items-start p-3.5 rounded-xl cursor-pointer border",
        consent ? "border-primary bg-primary/5 dark:bg-primary/10" : "border-border",
      )}
    >
      <input
        type="checkbox"
        ...
        className="mt-0.5 h-4 w-4 shrink-0 accent-[#FF5C35]"
      />
...
function AccountStep({
  ...
  const inputCls =
    "flex h-11 w-full rounded-xl border border-border bg-background pl-10 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent";
  return (
    <>
      <StepHeading
        title="Crea tu cuenta"
        sub="Empieza gratis. Luego verificamos tu identidad y aseguramos tu cuenta."
      />
      <div className="space-y-4">
        ...
        {password.length > 0 && (
          <div className="grid grid-cols-2 gap-1">
            {[
              ["8 caracteres", pwdIssues[0]],
              ["Una mayúscula", pwdIssues[1]],
              ["Una minúscula", pwdIssues[2]],
              ["Un número", pwdIssues[3]],
            ].map(([t, ok]) => (
              <div
                key={t as string}
                className={cn(
              ...
              >
                <CheckCircle2
                  className={cn(
                    "h-3 w-3 shrink-0",
                    ok ? "text-green-500" : "text-muted-foreground/50",
                  )}
                />{" "}
                {t as string}
              </div>
            ...
            {confirm.length > 0 && (
              <div
                className={cn(
                  "flex items-center gap-1.5 text-xs col-span-2",
                  pwdMatch ? "text-green-600" : "text-red-500",
                )}
              >
                <CheckCircle2
                  className={cn("h-3 w-3 shrink-0", pwdMatch ? "text-green-500" : "text-red-400")}
                />{" "}
  ...
  return (
    <>
      <StepHeading
        ...
        sub="Revisa cómo usamos tus datos y autoriza el tratamiento para prestarte el servicio."
      />
      <div className="border border-border rounded-xl p-4 max-h-36 overflow-y-auto bg-muted/40 text-[13px] text-muted-foreground leading-relaxed space-y-2">
        <p>
          <b className="text-foreground/80">1. Servicio.</b> CODA analiza la información que cargas
          para calcular tus indicadores de salud financiera y recomendarte productos. El uso es
          gratuito para ti.
        </p>
        <p>
          <b className="text-foreground/80">2. Datos.</b> Tratamos tus datos personales solo en la
          medida necesaria para prestar el servicio, conforme a la normativa vigente.
        </p>
        <p>
          <b className="text-foreground/80">3. Tus derechos.</b> Puedes revocar tu autorización y
...
function KycStep({
  ...
  back: boolean;
  bio: boolean;
  setFront: (v: boolean) => void;
  setBack: (v: boolean) => void;
  ...
  return (
    <>
      <StepHeading
        ...
        sub="Se realiza una sola vez. Es requisito para operar con datos bancarios."
      />
      <p className="text-[12px] text-muted-foreground mb-3 flex items-center gap-1.5">
        <Lock size={12} /> Tus imágenes se cifran · una única vez
      </p>
      <div className="grid gap-2.5">
        ...
        <CaptureCard
          ...
          hint="Selfie con prueba de vida"
          done={bio}
          onClick={() => setBio(true)}
        />
      </div>
...
function CaptureCard({
  ...
  return (
    ...
    >
      <span
        className={cn(
          "w-10 h-10 rounded-xl flex-shrink-0 grid place-items-center",
      ...
      <span>
        ...
        <span className={cn("text-xs", done ? "text-green-600" : "text-muted-foreground")}>
          {done ? "Capturado" : hint || "Tomar foto"}
        </span>
      </span>
...
function TwoFAStep({
  ...
  twoFA: null | "now" | "later";
  setTwoFA: (v: "now" | "later") => void;
  totpDone: boolean;
  setTotpDone: (v: boolean) => void;
}) {
  return (
    <>
      <StepHeading
        title="Doble factor (2FA)"
        sub="Protege tu cuenta con un segundo factor mediante una app de autenticación."
      />
      <div className="flex gap-2.5 items-start p-3 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 mb-4">
        ...
        <span className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
          Es <b>obligatorio</b> antes de vincular cuentas bancarias. Puedes activarlo ahora o más
          tarde.
        </span>
      </div>
      <div className="grid gap-2.5">
        <ChoiceCard
          Icon={KeyRound}
          title="Activar ahora"
          hint="App de autenticación (TOTP)"
          active={twoFA === "now"}
          onClick={() => setTwoFA("now")}
        />
        <ChoiceCard
          Icon={Smartphone}
          title="Más tarde"
          hint="Te lo pediremos antes de conectar un banco"
          active={twoFA === "later"}
          onClick={() => {
            setTwoFA("later");
            setTotpDone(false);
      ...
      {twoFA === "now" && (
        <div className="mt-4 p-4 rounded-xl border border-border bg-muted/40">
          <p className="text-[13px] text-muted-foreground mb-3">
            Escanea el código con tu app (Google/Microsoft Authenticator) e ingresa los 6 dígitos.
          </p>
          <div className="flex gap-4 items-center">
            <div className="w-[88px] h-[88px] rounded-xl bg-background border border-border grid place-items-center">
              ...
              <div className="flex gap-1.5">
                {[0, 1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className={cn(
                      "flex-1 h-9 rounded-lg border grid place-items-center font-bold text-foreground",
                      totpDone ? "border-green-500" : "border-border",
                    )}
                  >
                    {totpDone ? "•" : ""}
                  </div>
...
function ChoiceCard({
  ...
  return (
    <button
      ...
      className={cn(
        "w-full flex items-center gap-3 p-3.5 rounded-xl text-left transition-all border",
        active ? "border-primary bg-primary/5 dark:bg-primary/10" : "border-border",
      )}
    >
      <span
        className={cn(
      ...
      <span
        className={cn(
          "ml-auto w-5 h-5 rounded-full flex-shrink-0 border-2 grid place-items-center",
