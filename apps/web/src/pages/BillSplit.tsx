    ...
    <Card className="bg-gradient-to-br from-slate-900 to-slate-800 text-white">
      <CardContent className="p-6">
        ...
        <div className="text-center mb-6">
          <p
            className={`text-4xl font-bold ${netBalance >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {netBalance < 0 && "-"}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white/10 rounded-lg p-3">
            ...
            <p className="text-xl font-semibold text-green-400">
              {formatAmount(totalOwedToYou, currency)}
            </p>
          </div>
        </div>
      </CardContent>
        ...
        <div>
          ...
          >
            {isOwed
              ...
              : isOwing
                ? `le debes ${formatAmount(balance, currency)}`
                : "al día"}
          </p>
        </div>
      </div>
          ...
          <div className="flex-shrink-0 flex flex-col items-end gap-2 min-w-fit">
            <div className="text-right">
              <p className="text-2xl font-bold">
                {formatAmount(parseFloat(String(expense.totalAmount)) || 0, currency)}
              </p>
            </div>
              ...
              {userParticipant &&
                ...
                expense.status !== "settled" && (
                  <Badge variant="destructive" className="text-xs whitespace-nowrap">
                    Debes{" "}
                    {formatAmount(parseFloat(String(userParticipant.amountOwed)) || 0, currency)}
                  </Badge>
                )}
...
export default function BillSplit({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const { getBillSplits, createBillSplit, markParticipantAsPaid, deleteBillSplit, scanExpense } =
    useApi();
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery<BillSplitWithParticipants[]>({
  ...
  const billSplits = Array.isArray(billSplitsRaw)
    ? billSplitsRaw
    : billSplitsRaw
      ? [billSplitsRaw]
      : [];
    ...
    const participants = (split.participants ?? []).map((p: any, i: number) => ({
      ...
      amountOwed:
        typeof p.amountOwed === "number"
          ? p.amountOwed
          : parseFloat(String(p.amountOwed ?? p.amount_owed ?? 0)) || 0,
  ...
  const activeExpenses = billSplits.filter(
    (s) => s.status === "active" || s.status === "pending" || !s.status,
  );
  const settledExpenses = billSplits.filter(
    (s) => s.status === "settled" || s.status === "fully_paid",
  );
  ...
  const calculateBalances = (splits: BillSplitWithParticipants[]) => {
    ...
    (splits || []).forEach((split) => {
      ...
      if (
        ourParticipation &&
        !(ourParticipation.isPaid ?? (ourParticipation as { is_paid?: number }).is_paid) &&
        !isCreator
      ) {
        const creatorKey = String(split.createdBy ?? "creator");
        const rawOwed =
          ourParticipation.amountOwed ?? (ourParticipation as { amount_owed?: number }).amount_owed;
        const amount = Number(rawOwed) || parseFloat(String(rawOwed ?? 0)) || 0;
        if (!balances[creatorKey])
          balances[creatorKey] = { name: split.createdByName || "Creador del gasto", balance: 0 };
  ...
  const calculateTotals = (splits: BillSplitWithParticipants[]) => {
    ...
    (splits || []).forEach((split) => {
        ...
        (split.participants || []).forEach((p: BillSplitParticipantWithUser) => {
          const isMe =
            ...
            (p.isCurrentUser !== false &&
              ...
              String(p.userId) === String(currentUserId));
          if (isMe) return;
          if (!(p.isPaid ?? (p as { is_paid?: number }).is_paid)) {
            const rawOwed = p.amountOwed ?? (p as { amount_owed?: number }).amount_owed;
      ...
      if (!isCreator) {
        ...
        if (
          myParticipation &&
          !(myParticipation.isPaid ?? (myParticipation as { is_paid?: number }).is_paid)
        ) {
          const rawOwed =
            myParticipation.amountOwed ?? (myParticipation as { amount_owed?: number }).amount_owed;
  ...
  const handleBillScan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(file.type)) {
      toast({
        ...
        description: "Usa una imagen JPG o PNG.",
        variant: "destructive",
      });
      return;
    }
    ...
    return (
      <div
        className={
          embedded
            ? "w-full space-y-6"
            : "w-full max-w-screen-2xl mx-auto px-4 sm:px-6 py-8 space-y-6"
        }
      >
              ...
              <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                ...
                <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    ...
                    <DialogDescription>Divide una cuenta con amigos</DialogDescription>
                  </DialogHeader>
                  <Form {...form}>
                        ...
                        <Card className="bg-muted/50 border-dashed">
                          <CardContent className="p-4">
                            <p className="text-sm font-medium mb-2">
                              Vista previa del reparto (igual)
                            </p>
                            <div className="space-y-1.5">
                              {(() => {
                                ...
                                const creatorName =
                                  ...
                                  (user as any)?.name ||
                                  "Tú";
                                const participantsList = Array.isArray(watchParticipants)
                                  ? watchParticipants
                                  : [];
                                ...
                                const allNames = [
                                  `${creatorName} (tú)`,
                                  ...participantsList.map((p) => p.name || "Persona"),
                                ];
                              ...
                              <div className="flex justify-between text-sm font-medium">
                                <span>Total</span>
                                <span>
                                  {formatAmount(parseFloat(watchAmount || "0"), currency)}
                                </span>
                              </div>
                            </div>
                          </CardContent>
            ...
            <TabsList className="grid w-full grid-cols-3">
              ...
              <TabsTrigger value="friends" className="gap-2">
                ...
                {userBalances.filter((b) => b.balance !== 0).length > 0 && (
                  <Badge variant="secondary" className="ml-1 h-5 px-1.5">
                    {userBalances.filter((b) => b.balance !== 0).length}
                  </Badge>
                )}
              </TabsTrigger>
              ...
              {settledExpenses.length === 0 ? (
                <Card className="border-dashed">
                  <CardContent className="p-12 text-center">
                    ...
                    <p className="text-muted-foreground">Los gastos saldados aparecerán aquí</p>
                  </CardContent>
                </Card>
              ) : (
                <div className="space-y-3">
                  ...
                  <div className="space-y-6">
                    <div className="text-center py-4 bg-muted/50 rounded-lg">
                      ...
                      <p className="text-4xl font-bold">
                        {formatAmount(
                          parseFloat(String(selectedExpense.totalAmount)) || 0,
                          currency,
                        )}
                      </p>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              navigator.clipboard.writeText(
                                `${window.location.origin}${rutaDividirPublico(selectedExpense.shareCode ?? "")}`,
                              );
                              toast({
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              }).format(Number(selectedExpense.totalAmount));
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  {p.isPaid
                                    ? `Pagado ${formatAmount(parseFloat(String(p.amountPaid || p.amountOwed)) || 0, currency)}`
                                    : `Debe ${formatAmount(parseFloat(String(p.amountOwed)) || 0, currency)}`}
                                </p>
                              </div>
                            </div>
                            {p.isPaid ? (
