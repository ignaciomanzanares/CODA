  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg flex flex-col max-h-[90vh]">
        ...
        <div className="space-y-4 py-2 overflow-y-auto min-h-0 flex-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {/* Drop zone */}
          <div
            onDragOver={(e) => {
              e.preventDefault();
          ...
          {files.length > 0 && (
            <div
              className="space-y-2 max-h-56 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
              aria-live="polite"
            >
              {files.map((fs, idx) => (
                <div
