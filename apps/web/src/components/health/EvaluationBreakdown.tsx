...
const SEMAFORO_STYLES: Record<Semaforo, { dot: string; bg: string; text: string; label: string }> = {
  sano:    { dot: 'bg-emerald-500', bg: 'bg-emerald-50',  text: 'text-emerald-700', label: 'Saludable' },
...
function semaforoDeudaFlujo(ratio: number): Semaforo {
  if (ratio < 0.30) return 'sano';
  if (ratio <= 0.50) return 'alerta';
  return 'critico';
}

function semaforoDeudaActivos(ratio: number): Semaforo {
  if (ratio < 0.40) return 'sano';
  if (ratio <= 0.80) return 'alerta';
  return 'critico';
}

function semaforoAhorro(ratio: number): Semaforo {
  if (ratio >= 0.20) return 'sano';
  if (ratio >= 0.10) return 'alerta';
  return 'critico';
}
