    ...
    for (const fs of pending) {
      ...
      try {
        ...
        setFiles((prev) =>
          prev.map((f) =>
            f.file === fs.file
              ? { ...f, status: "success", message: "Cartola procesada correctamente" }
              : f,
          ),
        );
        successCount++;
        ...
        setFiles((prev) =>
          prev.map((f) => (f.file === fs.file ? { ...f, status: "error", message: msg } : f)),
        );
      }
    }
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            o haz clic para seleccionar · Múltiples archivos permitidos
          </p>
        </div>
        <input
          ref={inputRef}
