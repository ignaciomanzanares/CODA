...
import { MockProvider, type OBProvider } from "../connectors/openbanking/mockProvider";
import type { InsertAccount, InsertBalance, InsertTransaction } from "@shared/schema";
...
export async function ingestOpenBankingForUser(userId: string, provider: OBProvider = new MockProvider()) {
  ...
  for (const pa of pAccounts) {
    ...
    const items: InsertTransaction[] = pTxs.map((t) => ({
      accountId: created.id as number,
