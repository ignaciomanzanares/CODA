...
if (isProd && !process.env.DATABASE_URL) {
  throw new Error(
    "❌ DATABASE_URL is required in production. Set it in your environment variables.",
  );
}
...
if (isProd) {
  ...
  throw new Error(
    "❌ Do not use @coda/db (sqlite) in production. Use the API backend for all DB access.",
  );
} else {
